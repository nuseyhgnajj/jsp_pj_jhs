			...
			<div id = "section">
				<div class = "join"> 
					<form name = "modifyform" action="${path}/modifyCustomerAction.do" method="post">
						...
						if(selectCnt == 1){	//인증이 성공할때만 테이블에 작성합니다
						%>
							<table>
								<tr>
									<th>ID</th>
								...
								<tr>
									<th>이름</th>
									<td colspan = "2">
										<input type = "text" class ="input" name = "name" size="20" placeholder="20자 이내로 작성" required value= "<%= dto.getName()%>">
									</td>
								</tr>
								<tr>
									<th>생년월일</th>
									<td colspan = "2">
										<input type = "date" class ="input" name = "birthday" size="8" placeholder="생년월일 8자리" required value= "<%= dto.getBirthday()%>">
									</td>
								</tr>
								<tr>
									<th>주소</th>
								...
								<tr>
									...
									<td>
									<%							
										if(dto.getHp() ==null){		//dto.getHp()값을 분배해줍니다 
									%>
									...
									<% } 
										else {				// hp가 null이 아니면 "-"을 기준으로 삼등분해서 배열에 담아 뿌려줍니다
											String hp = dto.getHp();
